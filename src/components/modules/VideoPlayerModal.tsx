...
import { saveVideoProgress, getVideoProgress } from '../../services/dataService';
    ...
    if (videoRef.current) {
      if (isPlaying) {
        ...
        videoRef.current.play().catch(() => {
          // In sandboxed environments audio autoplay might require user interaction
          setIsPlaying(true);
        });
      }
          ...
          {activeTab === 'transcript' && (
            <div className="space-y-3 text-xs text-[#0F172A] leading-relaxed">
              <div className="p-3.5 rounded-xl bg-[#F7F9FB] border border-[#E2E8F0]">
                <div className="font-semibold text-[#1E3A5F] mb-1">Key Takeaways for Wednesday’s Lab:</div>
                <ul className="list-disc pl-4 space-y-1 text-[#5B6B7C]">
                  <li>Presentation tier should strictly format UI and validate form inputs; it must never execute SQL queries.</li>
                  <li>Backend Application tier issues signed pre-authorized upload tokens to offload heavy video traffic to object storage.</li>
                  <li>During class, groups will refactor a monolithic codebase into decoupled service layers.</li>
                </ul>
              </div>
              <p className="text-[#5B6B7C]">
                {video.transcript || 'In this lecture, we examine 3-Tier architecture and practical flipped classroom pedagogy.'}
              </p>
            </div>
          )}
              ...
              {materials.map((mat) => (
                ...
                >
                  ...
                  <a
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      addToast(`Downloading ${mat.title}`, 'info');
                    }}
                    className="px-3 py-1.5 bg-white border border-[#E2E8F0] hover:bg-[#EEF3F7] rounded-lg text-xs font-semibold text-[#1E3A5F] flex items-center gap-1.5 transition"
                  >
